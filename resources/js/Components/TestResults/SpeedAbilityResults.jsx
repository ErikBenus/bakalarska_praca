...
const SpeedAbilityResults = ({ clientId }) => {
    ...
    useEffect(() => {
        axios.get(`/api/tests/rychlostne-schopnosti?client_id=${clientId}`)
            .then(response => {
                setTests(response.data);

                const promises = response.data.map(test =>
                    axios.get(`/api/tests/rychlostne-schopnosti/${test.id}?client_id=${clientId}`)
                        .then(response => {
...
export default SpeedAbilityResults;
